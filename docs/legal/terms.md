# Terms of Service — Mental Models

_Last updated: 2026-04-27_

By using Mental Models ("the App"), you agree to these Terms.

## The service

Mental Models is a mobile app that delivers daily editorial content about mental models, cognitive biases, paradoxes, and thought experiments. It is provided as-is for personal, non-commercial use.

## Account

You may use the App without an account. If you sign in (email or Google), you agree that:
- The information you provide is accurate
- You are at least 13 years old
- You will not share your account or use someone else's account

## Content ownership

- All content (cards, illustrations, text) is © Mental Models / its authors. You may read it, share it via the in-app Share button, or quote short passages with attribution. You may not republish or reproduce content commercially.
- Your votes on scenarios become part of aggregated public statistics shown in the App. We do not associate them with your identity.

## Subscriptions (when launched)

If you purchase a subscription:
- Charges are handled by Apple App Store or Google Play, not directly by us
- Auto-renewal can be canceled in your store settings
- Refunds follow the store's policy
- We may change prices with 30 days notice

## Acceptable use

You may not:
- Reverse-engineer the App for redistribution
- Attempt to access other users' data
- Spam Firestore with automated votes
- Use the App in violation of applicable law

App Check (Play Integrity / App Attest) is used to enforce these rules.

## Disclaimer

The App provides general educational content. It is not professional advice (psychological, financial, medical). Decisions you make based on App content are your own.

## Liability

To the extent permitted by law, we are not liable for indirect or consequential damages arising from use of the App. Our liability is capped at the amount you paid us in the 12 months prior.

## Changes

We may update these Terms. Continued use after changes means acceptance.

## Contact

Email: TODO@example.com (will be filled before public release)
